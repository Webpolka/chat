  ...
  return (
    <div className="users-list p-2 w-64 bg-[#111] text-white overflow-y-auto">
      {Object.values(users)
        ...
        .map(u => (
          <div key={u.id} className="flex items-center justify-between p-1 hover:bg-[#222] rounded">
            <span>{u.username}</span>
            <span className={`h-3 w-3 rounded-full ${u.online ? "bg-green-500" : "bg-gray-500"}`} />
          </div>
        ))}
